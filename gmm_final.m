...
area_1(20) = sum(sum(t20));
area_1(21) = sum(sum(t21));
area_1(22) = sum(sum(t22));

dist = [68;76;91;99;106;114;121;137;144;152;160;168;176;192;200;208;216;223;231;248;256;264];
...
% max1 = max(max(probM))
% area_test = size(probM(probM >= max1/100),1);
% dist = fit_data(area_test)
% % test
% min1 = min(min(probM))
% 
% runimage(probM, covk, dist, meank, k)
...
function runimage(matrix,covk,dist, meank, k)
figure
mycolors =  [0 0 0.5; 1 0.5 0];
final_image = image(matrix,'CDataMapping','scaled')
colormap(mycolors);
colorbar
caxis([10.^(-9) 10.^(-6)])
